...
import { Heart, MessageCircle, Share2, Calendar, Send, Trash2 } from 'lucide-react';
...
export default function PostCard({ post, currentUser }) {
    ...
    try {
      if (isLiked) {
        ...
        if (post.user_id !== currentUser.id) {
          const actorName = currentUser.email?.split('@')[0] || 'Someone';
  ...
  const handleAddComment = async (e) => {
    ...
    try {
      ...
      if (post.user_id !== currentUser.id) {
        const actorName = data.profiles?.username || currentUser.email?.split('@')[0] || 'Someone';
        await supabase.from('notifications').insert([{
          user_id: post.user_id,
          actor_name: actorName,
          action_type: 'comment'
  ...
  return (
    <div className={`overflow-hidden md:w-3xl rounded-2xl border shadow-xl transition-all ${
      ...
      {/* Header */}
      <div className={`flex items-center gap-3 border-b p-4 ${isDarkMode ? 'border-slate-700/50' : 'border-slate-100'}`}>
        {profile.avatar_url ? (
        ...
        <div>
          <p className={`text-sm font-semibold ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>{displayName}</p>
